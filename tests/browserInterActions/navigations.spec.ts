...
test.describe('Google Test Group', () => {
    ...
    test('test case #1 - Validate URL and title Page', async ({ page, context }) => {
        ...
        await test.step('When I navigate to Google homepage and validat url , title', async () => {
    ...
    test('test case #3 - Validate login Functionality', async ({ page, context }) => {
        ...
        await test.step('When I navigate to Google homepage', async () => {
            await page.goto('https://my.dentolize.com/auth/login');
            await page.reload();
            expect(page).not.toHaveTitle('');
            expect(page).not.toHaveURL('');
        });
