...
test.describe(
  'Example to demonstrate execution Upload and Download single file methods in Playwright',
  { tag: '@examples' },
  () => {
    ...
    test('Download a Single file and assert', async ({ page }) => {
      ...
      const [download] = await Promise.all([page.waitForEvent('download'), page.locator(`text=${fileName}`).click()]);
...
test.describe(
  'Example to demonstrate execution Upload and remove multipe file methods in Playwright',
  { tag: '@examples' },
  () => {
    const fileName1 = 'example.json';
    const fileName2 = 'Screenshot.png';
